...
"""Check that the distribution and dancerunaway/__init__.py are in sync."""
...
def main() -> int:
    ...
    if setup_py_map["version"] != dancerunaway.__version__:
        print(
            f"The version in the setup.py is {setup_py_map['version']}, "
            f"while the version in dancerunaway/__init__.py is: "
            f"{dancerunaway.__version__}",
            file=sys.stderr,
        )
        success = False

    if setup_py_map["author"] != dancerunaway.__author__:
        print(
            f"The author in the setup.py is {setup_py_map['author']}, "
            f"while the author in dancerunaway/__init__.py is: "
            f"{dancerunaway.__author__}",
            file=sys.stderr,
        )
        success = False

    if setup_py_map["license"] != dancerunaway.__license__:
        print(
            f"The license in the setup.py is {setup_py_map['license']}, "
            f"while the license in dancerunaway/__init__.py is: "
            f"{dancerunaway.__license__}",
            file=sys.stderr,
        )
        success = False

    if setup_py_map["description"] != dancerunaway.__doc__:
        print(
            f"The description in the setup.py is {setup_py_map['description']}, "
            f"while the description in dancerunaway/__init__.py is: "
            f"{dancerunaway.__doc__}",
            file=sys.stderr,
        )
        success = False
    ...
    if status_classifier is None:
        ...
        if expected_status_in_init != dancerunaway.__status__:
            print(
                f"Expected status {expected_status_in_init} "
                f"according to setup.py in dancerunaway/__init__.py, "
                f"but found: {dancerunaway.__status__}"
            )
            success = False
